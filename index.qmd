...
author: "Rafael Farias Varona (DNI: 00000000-X), Sara García Bustos (DNI: 05439198-C), Manuel González Aramburu (DNI: 02576803-K) y Miguel Salmerón Vicente (DNI: 02769472-L)"
format:
  revealjs:
    theme: [style.scss]
...
## Municipios que replican el resultado de las encuestas (estimación global excluyendo a otros)
...
mapa_encuestas <- function(periodo){
  ...
  dataa <- 
    elect_surv_desag |> 
    filter(periodo_electoral == periodo) |> 
    mutate(diferencia_media_clac = diferencia_media,
           diferencia_media = if_else(diferencia_media <= quantile(diferencia_media, 0.9),
                                      diferencia_media, 
                                      NA))
  ...
  labs(fill = "",
       title = glue("Diferencia media entre el resultado en cada municipio\ny estimación de la intención de voto para el periodo electoral de\n{periodo}."),
       subtitle = paste0("Mediana global (q1, q3): ", round(median(dataa$diferencia_media_clac, na.rm = T), digits = 2), " (", round(quantile(dataa$diferencia_media_clac, 0.25, na.rm = T), digits = 2), ", ", round(quantile(dataa$diferencia_media_clac, 0.75, na.rm = T), digits = 2), ")"),
       caption = "se eliminan el 10% de los datos más extremos") +
  scale_fill_gradient2(high= "#ffffff",
                        mid = "#f28aaa",
                        low = "#9a133d") +

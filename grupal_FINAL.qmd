...
Se considera [**zona rural**]{style="color:red;"} aquella que tiene [**menos de 30.000 habitantes**]{style="color:red;"} y una densidad [**inferior a 100 habitantes por kilómetro cuadrado**]{style="color:red;"}
...
municipios_superficie <- read_xls(path = "municipios_superficie.xls")

superficie_tidy <- municipios_superficie |> distinct("DENOMINACION", .keep_all = TRUE)

election_superficie <- election_tidy |> left_join(municipios_superficie, by = c("municipio" = "DENOMINACION"))

rural <-
  election_superficie |> 
  filter(censo < 30000 & HABITANTES / SUPERFICIE < 100)
